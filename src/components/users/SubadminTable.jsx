...
import SubadminAccessModal from "./SubadminAccessModal";
...
const SubadminTable = () => {
  ...
  async function handleDelete(id) {
    ...
    const response = await axios.delete(
      `${import.meta.env.VITE_BASE_URL}/admin/employee/${id}`,
      {
        headers: {
          Authorization: `Bearer ${authToken}`,
          "Content-Type": "application/json",
  ...
  return (
    ...
    >
      <SubadminAccessModal isOpen={isModalOpen} onClose={closeModal} subadminId={selectedSubadminId} />
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold text-gray-100">Subadmins</h2>
          ...
          <tbody className="divide-y divide-gray-700">
            {filteredStores.map((store) => (
              ...
              >
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  {store.accessTo && (
                    <div className="text-sm text-gray-300">{Object.keys(store.accessTo).join(", ")}</div>
                  )}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                  <button
                    className="text-i</td>ndigo-400 hover:text-indigo-300 mr-6"
                    onClick={() => openSubadminModal(store.id)}
                  >
                    Edit
                  </button>
                  <button
